...
static void lge_bc_dim_set_rm692C9(struct dsi_panel *panel, u8 bc_dim_en, u8 bc_dim_f_cnt)
{
	char *payload = NULL;
	u8 data = 0;
...
static int lge_set_therm_dim_rm692C9(struct dsi_panel *panel, int input)
{
	u8 bc_dim_f_cnt;
	struct backlight_device *bd;

	if (!panel->bl_config.raw_bd) {
		pr_err("bl device is NULL\n");
		return -EINVAL;
	}
	...
	lge_bc_dim_set_rm692C9(panel, BC_DIM_ON, bc_dim_f_cnt);
...
static int lge_get_brightness_dim_rm692C9(struct dsi_panel *panel)
{
	u8 bc_dim_f_cnt;
	char *payload = NULL;
...
static void lge_set_brightness_dim_rm692C9(struct dsi_panel *panel, int input)
{
	u8 bc_dim_f_cnt;

	if (input == BC_DIM_OFF) {
		pr_warn("rm692C9 do not off bc_dim feature\n");
		bc_dim_f_cnt = BC_DIM_FRAMES_NORMAL;
	} else {
...
struct lge_ddic_ops rm692C9_ops = {
	...
	.lge_bc_dim_set = lge_bc_dim_set_rm692C9,
	.lge_set_therm_dim = lge_set_therm_dim_rm692C9,
	.lge_get_brightness_dim = lge_get_brightness_dim_rm692C9,
	.lge_set_brightness_dim = lge_set_brightness_dim_rm692C9,
	/* To Do : Image Quality */

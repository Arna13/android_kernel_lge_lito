...
static void set_actm_current(int set_curr)
{
    ...
    if (actm_me.conn == CHARGING_SUPPLY_WIRELESS_9W ||
        ...
        if (set_curr > 7200 && actm_me.max_volt > 0)
...
static bool actm_parse_dt(struct device_node* dnode)
{
    ...
    pr_actm(UPDATE,
        ...
        "idtp9222 default: max=%dmV, epp=%dmV, bpp=%dmV, lcdon-temp-offset: %d\n",
        actm_me.enable,
        actm_me.enable_on_chargerlogo,
        ...
        actm_me.cap_min, actm_me.max_volt, actm_me.epp_volt, actm_me.bpp_volt,

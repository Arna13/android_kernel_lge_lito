...
static void protection_batvolt_work(struct work_struct *work)
{
	...
	if (bvp_me.is_three_step_batt && ui_soc > bvp_me.three_step_max_soc) {
		pr_batvolt(UPDATE,
			"soc now = %d(limit=%d), icap_new=%d(limit=%d)\n",
				ui_soc, bvp_me.three_step_max_soc,
				icap_new, bvp_me.threshold_ibat_rated[0]);
		icap_new = min(icap_new, bvp_me.threshold_ibat_rated[0]);
	}

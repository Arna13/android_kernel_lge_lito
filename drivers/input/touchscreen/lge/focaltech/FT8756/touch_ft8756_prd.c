...
static char line[50000]; //need optimize
...
int ft8756_change_op_mode(struct device *dev, u8 op_mode)
{
	...
	if((op_mode == FTS_FACTORY_MODE) && (d->state == TC_STATE_LPWG)) {
		...
		data = 0x01;
		ret = ft8756_reg_write(dev, 0x88, &data, 1);
		if(ret < 0) {
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	if ((rawdata_ret == TEST_PASS) && (noise_ret == TEST_PASS)) {
		...
		ret += touch_snprintf(log_buf + ret, sizeof(log_buf) - ret, "Raw Data : Fail (raw:%d/noise:%d)\n",
	...
	if (cb_ret == TEST_PASS) {
		...
		ret += touch_snprintf(log_buf + ret, sizeof(log_buf) - ret, "Channel Status: Fail (cb:%d)\n",
				(cb_ret == TEST_FAIL)? 0 : 1);
		TOUCH_I("Channel Status : Fail (cb:%d)\n",
				(cb_ret == TEST_FAIL)? 0 : 1);
	}
		...
		char *buf, loff_t off, size_t count)
{
	...
	if (off == 0) {
		...
		// Data Select to raw data
		data = 0x01;
		ret = ft8756_reg_write(ts->dev, 0x06, &data, 1);

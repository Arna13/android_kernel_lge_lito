...
void ft8756_dummy_read(struct device *dev)
{
	u8 dummy = 0x00;
	...
	ft8756_reg_read(dev, 0x00, &dummy, 1);
	touch_msleep(40);
...
static int ft8756_deep_sleep(struct device *dev, int mode)
{
	...
	if(mode) {
		...
		return ft8756_reg_write(dev, 0xA5, &data, 1);
	}
	else {
...
#if 0
static void ft8756_debug_tci(struct device *dev)
{
	struct ft8756_data *d = to_ft8756_data(dev);
	...
	ft8756_reg_read(dev, TCI_DEBUG_R, &rdata, sizeof(rdata));
...
int ft8756_lpwg_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_I("ft8756_lpwg_reset\n");
	touch_gpio_direction_output(ts->reset_pin, 0);
	touch_msleep(5);
	touch_gpio_direction_output(ts->reset_pin, 1);
...
#if 0
static void ft8756_lcd_event_read_reg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u32 rdata[5] = {0};

//	ft8756_xfer_msg_ready(dev, 5);
...
void ft8756_xfer_msg_ready(struct device *dev, u8 msg_cnt)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8756_data *d = to_ft8756_data(dev);
...
static int ft8756_debug_tool(struct device *dev, u32 value)
{
	struct touch_core_data *ts = to_touch_core(dev);

	if (value == DEBUG_TOOL_ENABLE) {
//		ts->driver->irq_handler = ft8756_sic_abt_irq_handler;
	} else {
		ts->driver->irq_handler = ft8756_irq_handler;
	}
...
static int ft8756_debug_option(struct device *dev, u32 *data)
{
	...
	case DEBUG_OPTION_1:
//		if (enable)	/* ASC */
//			ft8756_asc_control(dev, ASC_ON);
//		else
//			ft8756_asc_control(dev, ASC_OFF);
		break;
	case DEBUG_OPTION_2:
...
int ft8756_debug_info(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8756_data *d = to_ft8756_data(dev);
	struct ft8756_touch_debug *debug = &d->info.debug;
	int ret = 0;
	int i = 0;
...
//report_debug_info:
		ret += touch_snprintf(buf + ret, DEBUG_BUF_SIZE - ret,
...
//	ft8756_watch_register_sysfs(dev);
	ft8756_prd_register_sysfs(dev);
//	ft8756_asc_register_sysfs(dev);	/* ASC */
//	ft8756_sic_abt_register_sysfs(&ts->kobj);
...
static int ft8756_swipe_enable(struct device *dev, bool enable)
{
	...
	if (d->state == TC_STATE_LPWG) {
		if (ts->swipe[SWIPE_L2].enable) {
			if (d->start_pay_area_L2.x1 != ts->swipe[SWIPE_L2].start_area.x1 ||
					d->start_pay_area_L2.x2 != ts->swipe[SWIPE_L2].start_area.x2 ||
					d->start_pay_area_L2.y1 != ts->swipe[SWIPE_L2].start_area.y1 ||
					d->start_pay_area_L2.y2 != ts->swipe[SWIPE_L2].start_area.y2) {
				...
				ret = ft8756_reg_read(dev, 0x0, &temp_data, 0); //temp bringup, read, because i2c block activation
			...
			if (d->start_pay_area_R2.x1 != ts->swipe[SWIPE_R2].start_area.x1 ||
						d->start_pay_area_R2.x2 != ts->swipe[SWIPE_R2].start_area.x2 ||
						d->start_pay_area_R2.y1 != ts->swipe[SWIPE_R2].start_area.y1 ||
						d->start_pay_area_R2.y2 != ts->swipe[SWIPE_R2].start_area.y2) {

...
static ssize_t show_sp_link_touch_sub_off(struct device *dev, char *buf)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	int ret = 0;
...
static ssize_t store_sp_link_touch_sub_off(struct device *dev,
		const char *buf, size_t count)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	int value = 0;
	...
	for (i = 0; i < 3; i++) {
		if (i == 2) {
			...
			ret += snprintf(buf + ret, PAGE_SIZE - ret, "%d %s %d %d\n",
					...
					ts->app_data[i].icon_size, ts->app_data[i].touch_sub_slop);
		}
		if (ts->app_data[i].icon_size != 0) {
			TOUCH_I("%s : Read %s App data (Icon_Size = %d, Touch_Slop = %d, Version = %s)\n",
					...
					ts->app_data[i].icon_size, ts->app_data[i].touch_sub_slop, ts->app_data[i].version);
		}
		...
		const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %10s %d %d",
			...
			&app_data_buf.icon_size, &app_data_buf.touch_sub_slop) <= 0)
		return count;
	...
	if (app_data_buf.app >= APP_HOME && app_data_buf.app <= APP_CONTACTS) {
		...
		TOUCH_I("%s : Write %s App data (Icon_Size = %d, Touch_Slop = %d, Version = %s)\n",
				...
				app_data_buf.icon_size, app_data_buf.touch_sub_slop, app_data_buf.version);
	}
...
#if defined(CONFIG_SUB_SECURE_TOUCH)
static ssize_t show_secure_touch_sub_devinfo(struct device *dev,
				char *buf)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	int ret = 0;
...
static ssize_t show_secure_touch_sub_enable(struct device *dev,
				char *buf)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
 ...
 */
static ssize_t store_secure_touch_sub_enable(struct device *dev,
		const char *buf, size_t count)
{
	struct touch_sub_core_data *ts = to_touch_sub_core(dev);
	unsigned long value;
...
static TOUCH_ATTR(sp_link_touch_sub_off,
	show_sp_link_touch_sub_off, store_sp_link_touch_sub_off);
...
#if defined(CONFIG_SUB_SECURE_TOUCH)
static TOUCH_ATTR(secure_touch_sub_enable, show_secure_touch_sub_enable,
						store_secure_touch_sub_enable);
...
static struct attribute *touch_sub_attribute_list[] = {
	...
	&touch_sub_attr_sp_link_touch_sub_off.attr,
...
#if defined(CONFIG_SUB_SECURE_TOUCH)
	&touch_sub_attr_secure_touch_sub_enable.attr,

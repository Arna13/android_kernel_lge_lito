...
static int sw42902_tci_control(struct device *dev, int type)
{
	...
	case TOUCH_SLOP_CTRL:
		...
		lpwg_data = info1->touch_sub_slop | (info2->touch_sub_slop << 16);
		ret = sw42902_sub_reg_write(dev, addr, &lpwg_data, sizeof(lpwg_data));
		break;
...
static ssize_t show_longpress(struct device *dev, char *buf)
{
	...
	TOUCH_I("%s: enable = %d, sensor_area(%d,%d)(%d,%d), min_press_time =%d, min_contact_size = %d, touch_sub_slop = %d\n",
...
static ssize_t show_onetap(struct device *dev, char *buf)
{
	...
	ret += touch_sub_snprintf(buf + ret, PAGE_SIZE, "%d\n",
			d->lpwg_onetap.enable);
	TOUCH_I("%s: enable = %d, active_area(%d,%d)(%d,%d), touch_sub_slop = %d, max_press_time = %d, interrupt_delay_time = %d\n",
...
*/
	TOUCH_I("%s, sw42902 detected: touch_sub_id:%d\n", __func__, maker);
	printk("[Touch Sub] JJH1\n");
